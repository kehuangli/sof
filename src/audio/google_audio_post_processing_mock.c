...
GoogleAudioPostProcessingState *GoogleAudioPostProcessingCreate()
{
	struct GoogleAudioPostProcessingState *s = rballoc(
		0, SOF_MEM_CAPS_RAM, sizeof(GoogleAudioPostProcessingState));
...
int GoogleAudioPostProcessingSetup(GoogleAudioPostProcessingState *const state,
				   ...
				   const uint8_t *const config, int config_size)
{
	if (config_size > sizeof(state->config))
		return -ENOMEM;

	memcpy_s(state->config, sizeof(state->config), config, config_size);
	return 0;
}
	...
	uint8_t *const config, int max_config_size)
{
	...
	memcpy_s(config, max_config_size, state->config, config_size);
	return config_size;
}
	...
	struct GoogleAudioPostProcessingBuffer *const dest)
{
	...
	for (n = 0; n < src->frames; ++n) {
		...
		if (r_ptr >= (int32_t *)src->end_addr) {
			r_ptr = (int32_t *)src->base_addr +
				(r_ptr - (int32_t *)src->end_addr);
		}
		if (w_ptr >= (int32_t *)dest->end_addr) {
			w_ptr = (int32_t *)dest->base_addr +
				(w_ptr - (int32_t *)dest->end_addr);
		}
	}
	return 0;
}
				    ...
				    const int *const volume, int num_channels)
{
	...
	if (num_channels > sizeof(state->volume))
		return -ENOMEM;
