...
DECLARE_SOF_RT_UUID("google-audio-post-processing", gapp_uuid, 0xfd48f000,
		    0xc316, 0x4ec2, 0x9f, 0xf8, 0xed, 0xb4, 0xef, 0xa3, 0xf5,
		    0x2c);
...
static int gapp_setup(struct comp_dev *dev, struct comp_data *cd)
{
	...
	return GoogleAudioPostProcessingSetup(cd->state, cd->num_channels,
					      dev->frames,
					      cd->channel_volume[0], config,
					      config_size);
}
...
static int gapp_set_volume(struct comp_data *cd)
{
	...
	if (cd->has_new_volume) {
		ret = GoogleAudioPostProcessingSetVol(
			cd->state, cd->channel_volume, cd->num_channels);
		if (ret >= 0)
...
static int gapp_ctrl_set_val(struct comp_dev *dev,
			     struct sof_ipc_ctrl_data *cdata)
{
	...
	case SOF_CTRL_CMD_VOLUME:
		comp_info(
			dev,
			"gapp_ctrl_set_val(): SOF_CTRL_CMD_VOLUME, comp_id = %u",
			cdata->comp_id);
		/* validate */
		if (cdata->num_elems > SOF_IPC_MAX_CHANNELS) {
			comp_err(
				dev,
				"gapp_ctrl_set_val(): invalid cdata->num_elems %u",
				cdata->num_elems);
			return -EINVAL;
		}
		for (j = 0; j < cdata->num_elems; j++) {
			...
			comp_info(
				dev,
				"gapp_ctrl_set_val(), channel = %d, value = %u",
				ch, val);
	...
	default:
		comp_err(
			dev,
			"gapp_ctrl_set_val(): Only volume control supported %d",
			cdata->cmd);
		return -EINVAL;
	}
}

static int gapp_ctrl_get_val(struct comp_dev *dev,
			     struct sof_ipc_ctrl_data *cdata, int max_data_size)
{
	struct comp_data *cd = comp_get_drvdata(dev);
	int j;
	...
	case SOF_CTRL_CMD_VOLUME:
		comp_info(
			dev,
			"gapp_ctrl_get_val(): SOF_CTRL_CMD_VOLUME, comp_id = %u",
			cdata->comp_id);
		/* validate */
		if (cdata->num_elems == 0 ||
			...
			comp_err(
				dev,
				"gapp_ctrl_get_val(): invalid cdata->num_elems %u",
				cdata->num_elems);
			return -EINVAL;
		}
		for (j = 0; j < cdata->num_elems; j++) {
	...
	default:
		comp_err(
			dev,
			"gapp_ctrl_get_val(): Only volume control supported %d",
			cdata->cmd);
		return -EINVAL;
	}
}

static int gapp_set_comp_config(struct comp_dev *dev,
				struct sof_ipc_ctrl_data *cdata)
{
	struct comp_data *cd = comp_get_drvdata(dev);

	if (cdata->data->size != sizeof(cd->config)) {
		comp_err(dev, "gapp_set_comp_config(): invalid data size %d",
			 cdata->data->size);
		return -EINVAL;
	}

	memcpy_s(cd->config, sizeof(cd->config), cdata->data->data,
		 cdata->data->size);
	comp_dbg(dev, "GAPP new settings c[0] %u c[1] %u", cd->config[0],
		 cd->config[1]);

	return 0;
}

static int gapp_ctrl_set_bin_data(struct comp_dev *dev,
				  struct sof_ipc_ctrl_data *cdata)
{
	struct comp_data *cd = comp_get_drvdata(dev);
	...
	comp_dbg(
		...
		cdata->data->type, cdata->comp_id, cdata->msg_index,
		cdata->num_elems);
	...
	default:
		comp_err(dev,
			 "gapp_ctrl_set_bin_data(): unknown binary data type");
		return -EINVAL;
	}
}

static int gapp_ctrl_set_data(struct comp_dev *dev,
			      struct sof_ipc_ctrl_data *cdata)
{
...
static int gapp_get_comp_config(struct comp_data *cd,
				struct sof_ipc_ctrl_data *cdata,
				int max_data_size)
{
	...
	memcpy_s(cdata->data->data, max_data_size, cd->config,
		 sizeof(cd->config));
...
static int gapp_get_internal_config(struct comp_data *cd,
				    struct sof_ipc_ctrl_data *cdata,
				    int max_data_size)
{
	int data_size;
	...
	data_size = GoogleAudioPostProcessingGetConfig(
		cd->state, cdata->data->type, cdata->msg_index,
...
static int gapp_ctrl_get_bin_data(struct comp_dev *dev,
				  struct sof_ipc_ctrl_data *cdata,
				  int max_data_size)
{
	struct comp_data *cd = comp_get_drvdata(dev);
	...
	comp_dbg(
		dev,
		"gapp_ctrl_get_bin_data(): type = %u, comp_id = %u, index = %u, blob_size = %d",
		cdata->data->type, cdata->comp_id, cdata->msg_index,
		cdata->num_elems);
	...
	default:
		comp_err(dev,
			 "gapp_ctrl_get_bin_data(): unknown binary data type");
		return -EINVAL;
	}
}

static int gapp_ctrl_get_data(struct comp_dev *dev,
			      struct sof_ipc_ctrl_data *cdata,
			      int max_data_size)
{
	switch (cdata->cmd) {
...
static int gapp_cmd(struct comp_dev *dev, int cmd, void *data,
		    int max_data_size)
{
...
static struct comp_dev *gapp_new(const struct comp_driver *drv,
				 struct sof_ipc_comp *comp)
{
	...
	ret = memcpy_s(gapp, sizeof(*gapp), ipc_gapp,
		       sizeof(struct sof_ipc_comp_process));
	if (!ret)
		goto fail;

	if (ipc_gapp->size == sizeof(cd->config)) {
		memcpy_s(cd->config, sizeof(cd->config), ipc_gapp->data,
			 ipc_gapp->size);
	}
	...
	comp_dbg(dev, "GAPP created c[0] %d c[1] %d", cd->config[0],
		 cd->config[1]);
...
static int gapp_params(struct comp_dev *dev,
		       struct sof_ipc_stream_params *params)
{
	...
	if (ret < 0) {
		comp_err(dev, "gapp_params(): comp_verify_params failed.");
		return ret;
	}
...
{
	comp_info(dev, "gapp_trigger(): cmd = %d", cmd);
...
static int gapp_prepare(struct comp_dev *dev)
{
	...
	sink_buf = list_first_item(&dev->bsink_list, struct comp_buffer,
				   source_list);

	sink_per_bytes =
		audio_stream_period_bytes(&sink_buf->stream, dev->frames);

	if (sink_buf->stream.size < config->periods_sink * sink_per_bytes) {
		comp_err(dev,
			 "gapp_prepare(): sink buffer size is insufficient");
		return -ENOBUFS;
	}
	...
	case SOF_IPC_FRAME_S16_LE:
		break;
	default:
		comp_err(dev, "unsupported data format: %d",
			 sink_buf->stream.frame_fmt);
		return -EINVAL;
	}

	if (sink_buf->stream.rate != GOOGLE_AUDIO_POST_PROCESSING_SAMPLERATE) {
		comp_err(dev, "unsupported samplerate: %d",
			 sink_buf->stream.rate);
		return -EINVAL;
	}
...
{
	comp_register(
		platform_shared_get(&comp_gapp_info, sizeof(comp_gapp_info)));
}
